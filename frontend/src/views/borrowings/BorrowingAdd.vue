...
const handleSubmit = async () => {
  ...
  loading.value = true
  try {
    await borrowingsApi.add({
      userId: form.value.userId,
      bookId: form.value.bookId,
    })
    ElMessage.success('Borrowing added successfully')
    router.push('/borrowings')
  } catch (error: any) {
    ElMessage.error(error.response?.data?.message || 'Failed to add borrowing')
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="borrowing-add" v-loading="loading">
    ...
    <el-form :model="form" label-width="100px" style="max-width: 600px; margin-top: 30px">
      <el-form-item label="User" required>
        <el-select v-model="form.userId" placeholder="Select user" style="width: 100%" filterable>
          <el-option
      ...
      <el-form-item label="Book" required>
        <el-select v-model="form.bookId" placeholder="Select book" style="width: 100%" filterable>
          <el-option
